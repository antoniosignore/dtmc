...
//    }
    ...
    public
    static XYGraph mergeXYGraphs(String graphtitle, Vector graphs, int w, int h, int weight, Image background, Date date, int type,
                                 double value) {
        GraphFactory graphfactory = GraphFactory.getInstance();
    ...
    public
    static FinGraph getXYGraph(String name, float[] series, int w, int h, int weight, boolean isAreachart, boolean isLogScale, double x, double y, String label) {
        return getXYGraph(name, series, w, h, weight, isAreachart, isLogScale, null, x, y, label);
    }

    public
    static FinGraph getXYGraph(String name, float[] series, int w, int h, int weight, boolean isAreachart, boolean isLogScale, Image background,
    ...
    public
    static FinGraph getXYGraph(String name, int[] axis, float[] series, int w, int h, int weight, boolean isAreachart, boolean isLogScale) {
    ...
    public static XYSeriesCollection getXYDataset(String name, float[] data) {
    ...
    public
    static FinGraph createGraph(String name, float[] series, int w, int h, int weight, boolean isLogScale, int size) {
        return createGraph(name, series, w, h, weight, false, null, 0.0D, 0.0D, null, size);
    }

    public
    static FinGraph createGraph(String name, float[] series, int w, int h, int weight, boolean isLogScale, double x,
                                double y, String label) {
        return createGraph(name, series, w, h, weight, false, null, x, y, label);
    }

    public
    static FinGraph createGraph(String name, float[] series, int w, int h, int weight, boolean isLogScale, Image background, double x, double y, String label, int size) {
        return getXYGraph(name, series, w, h, weight, false, isLogScale, background, x, y, label, size);
    }

    public
    static FinGraph createGraph(String name, float[] series, int w, int h, int weight, boolean isLogScale, Image background, double x, double y, String label) {
        return getXYGraph(name, series, w, h, weight, false, isLogScale, background, x, y, label);
    }

    public
    static FinGraph createGraph(String name, int[] axis, float[] series, int size, int w, int h, int weight, boolean isLogScale) {
        float[] copiedserie = Util.vector(1, size);
        for (int i = 1; i <= size; i++)
            copiedserie[i] = series[i];
...
//    }
    ...
    public static void addDateMarker(FinGraph graph, Date date, int type, double value) {
        ...
        double millis = date.getFirstMillisecond();
        CircleDrawer cd = null;
        XYAnnotation note = null;
        XYPointerAnnotation pointer = null;
        if (type == 8) {
            ...
            pointer = new XYPointerAnnotation(date.toString(), millis, value, 1.5707963267948966D);
            pointer.setTextAnchor(TextAnchor.HALF_ASCENT_CENTER);
            pointer.setPaint(Color.blue);
        } else if (type == 9) {
            cd = new CircleDrawer(Color.blue, new BasicStroke(1.0F), null);
            ...
            cd = new CircleDrawer(Color.red, new BasicStroke(1.0F), null);
            note = new XYDrawableAnnotation(millis, value, 11D, 11D, cd);
            pointer = new XYPointerAnnotation(date.toString(), millis, value, 4.7123889803846897D);
            pointer.setTextAnchor(TextAnchor.HALF_ASCENT_CENTER);
            pointer.setPaint(Color.red);
        } else if (type == 5) {
            cd = new CircleDrawer(Color.red, new BasicStroke(1.0F), null);
