...
public class Stochastics implements Serializable {
    ...
    public static int[] extremeValueKSignalPeriod(double extremeLow, double extremeHigh,
                                                  double[] highs, double[] lows, double[] closes, int noOfPeriods) {
        if (highs == null || highs.length == 0) throw new IllegalArgumentException("The array high is empty.");
        if (lows == null || lows.length == 0) throw new IllegalArgumentException("The array low is empty.");
        if (highs.length != lows.length || lows.length != closes.length || closes.length != highs.length)
        ...
        for (int j = ad3.length; --j >= 1;) ai[j - 1] = extremeValueSignal(extremeLow, extremeHigh, ad3[j], ad3[j - 1]);

        return ai;
    }

    public static int[] extremeValueDSignalPeriod(double extremeLow, double extremeHigh,
                                                  double[] highs,
                                                  double[] lows, double[] closes, int i, int j, int k) {
        if (highs == null || highs.length == 0) throw new IllegalArgumentException("The array high is empty.");
        if (lows == null || lows.length == 0) throw new IllegalArgumentException("The array low is empty.");
        if (highs.length != lows.length || lows.length != closes.length || closes.length != highs.length)
            throw new IllegalArgumentException("The lengths of the low and high and close arrays should be identical.");
        ...
        double[] ad3 = new double[(highs.length - i - k) + 1];
        ad3 = dStochastic(highs, lows, closes, i, j, k);
        int[] ai = new int[ad3.length - 1];
        for (int l = ad3.length; --l >= 1;) ai[l - 1] = extremeValueSignal(extremeLow, extremeHigh, ad3[l], ad3[l - 1]);
        return ai;
    }
