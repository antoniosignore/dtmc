...
/**
 * LU Decomposition. <P> For an m-by-n matrix A with m >= n, the LU decomposition is an m-by-n unit lower triangular
 * matrix L, an n-by-n upper triangular matrix U, and a permutation vector piv of length m so that A(piv,:) = L*U. If m
 * < n, then L is m-by-m and U is m-by-n. <P> The LU decompostion with pivoting always exists, even if the matrix is
 ...
 *\
   ...
   public LUDecomposition (GenericMatrix A, int linpackflag) {
      // Initialize.
      LU = A.getArrayCopy();
      m = A.getRowDimension();
      n = A.getColumnDimension();
 ...
 * ------------------------ */
     ...
     *
     * @return det ( A )
     * @throws IllegalArgumentException GenericMatrix must be square
     */
    public double det() {
        if (m != n) {
            throw new IllegalArgumentException("GenericMatrix must be square.");
        }
    ...
    public Matrix solve(Matrix B) {
        ...
        if (!this.isNonsingular()) {
            throw new RuntimeException("GenericMatrix is singular.");
        }
