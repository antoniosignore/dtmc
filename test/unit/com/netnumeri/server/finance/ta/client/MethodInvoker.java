...
// Referenced classes of package webcab.lib.util.example:
//            Type
...
public class MethodInvoker {
    ...
    public static String arrayToString(Object a, String delim, String language) {
        ...
        if ("java".equals(language)) {
            ...
            for (int ii = 0; ii < len; ii++) {
                sb.append(arrayToString(Array.get(a, ii), delim));
                if (ii < len - 1)
                    sb.append(delim);
            }
            ...
            Class cls = getArrayClass(a);
            sb.append("new " + getLanguageSpecificTypeName(cls.getName(), "cs"));
            int i = 0;
            for (int dim = getArrayDimension(a); i < dim; i++)
                sb.append("[]");
            ...
            for (int ix = 0; ix < len; ix++) {
                sb.append(arrayToString(Array.get(a, ix), delim, language));
                if (ix < len - 1)
                    sb.append(delim);
            }

            sb.append('}');
        } else if ("vb".equals(language)) {
            Class cls = getArrayClass(a);
            String clsN = getLanguageSpecificTypeName(cls.getName(), "vb");
            sb.append("New " + Character.toUpperCase(clsN.charAt(0)) + clsN.substring(1));
            int i = 0;
            for (int dim = getArrayDimension(a); i < dim; i++)
                sb.append("()");
            ...
            for (int it = 0; it < len; it++) {
                sb.append(arrayToString(Array.get(a, it), delim, language));
                if (it < len - 1)
                    sb.append(delim);
            }

            sb.append('}');
        } else if ("delphi".equals(language)) {
            sb.append("(");
            int len = Array.getLength(a);
            for (int i = 0; i < len; i++) {
    ...
    public static String methodString(String methodName, Class clss[], String names[], String language) {
        ...
        if ("java".equals(language)) {
            ...
            sb.append("" + Character.toUpperCase(methodName.charAt(0)) + methodName.substring(1));
            sb.append("( ");
            for (int i = 0; i < clss.length; i++) {
                String clsN = clss[i].getName();
                int firstOccurrence = clsN.indexOf('[');
    ...
    public static String getLanguageSpecificTypeName(String type, String language) {
        if (!"cs".equals(language) && !"vb".equals(language) && !"delphi".equals(language) && !"c".equals(language))
            throw new IllegalArgumentException("unknown language identifier");
        String s[] = (String[]) (String[]) languageSpecificTypeName.get(type);
        if (null != s) {
    ...
    public static String trunc(Object o) {
        if (o.getClass().equals(java.lang.Double.class)) {
            ...
            return "" + (double) (int) (10000D * d) / 10000D;
        }
        if (o.getClass().equals(java.lang.Float.class)) {
            double f = ((Float) o).floatValue();
            return "" + (double) (int) (10000F * f) / 10000D;
        } else {
            return o.toString();
        }
    }
