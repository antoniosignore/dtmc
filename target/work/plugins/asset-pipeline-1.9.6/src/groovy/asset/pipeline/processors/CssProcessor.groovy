...
class CssProcessor extends AbstractProcessor {
    ...
    private relativePathToBaseFile(file, baseFile, useDigest = false) {
        def baseRelativePath = relativePath(baseFile).split(AssetHelper.DIRECTIVE_FILE_SEPARATOR).findAll {
            it
        }.reverse()
        def currentRelativePath = relativePath(file, false).split(AssetHelper.DIRECTIVE_FILE_SEPARATOR).findAll({
            it
        }).reverse()
