...
)
{
    ...
    {
        DAO.query({appName: \$scope.appConfig.appName, token
    : \$scope.appConfig.token, controller
    :
        '${instance}', action
    :
        'list'
...
\$scope.loadingSite = true,
    function (result) {
        ...
        s = result;
        var putIt = params.sorting() ? \$filter('orderBy')(\$scope.$
        {
            instance
        }
        s, params.orderBy()
        ...
        id;
        putIt = params.filter ? \$filter('filter')(putIt, params.filter())
        :
        putIt;
        ...
        s = (putIt.slice((params.page() - 1) * params.count(), params.page() * params.count()));
...
$rootScope.getAll$className = function () {
    ...
    DAO.query({appName: \$rootScope.appConfig.appName, token
    : \$rootScope.appConfig.token, controller
    :
    '${instance}', action
    :
    'list'
...
$rootScope.save$className = function () {
    \$rootScope.errors.errorMessages = [];
    DAO.save({appName: \$rootScope.appConfig.appName, token
    : \$rootScope.appConfig.token, instance
    :\$rootScope.$
    {
        instance
    }
    ,
    controller:'${instance}', action
    :
    'save'
},
    function (result) {
...
$rootScope.update$className = function () {
    \$rootScope.errors.errorMessages = [];
    DAO.update({appName: \$rootScope.appConfig.appName, token
    : \$rootScope.appConfig.token, instance
    :\$rootScope.$
    {
        instance
...
$rootScope.edit$className = function (${instance}) {
    \$rootScope.errors.errorMessages = [];
    DAO.get({appName: \$rootScope.appConfig.appName, token
    : \$rootScope.appConfig.token, instance
    :\$rootScope.$
    {
        instance
...
$rootScope.confirmDelete$className = function () {
    \$rootScope.errors.errorMessages = [];
    DAO.delete({appName: \$rootScope.appConfig.appName, token
    : \$rootScope.appConfig.token, instance
    :\$rootScope.$
    {
        instance
