...
Timeplot._Impl.prototype = {dispose:function() {
...
},addPainter:function(B, A) {
    var D = this._painters[B];
    if (D) {
        for (var C = 0;
             C < D.length;
             C++) {
            if (D[C].context._id == A.context._id) {
                return;
            }
        }
        D.push(A);
    }
},removePainter:function(B, A) {
    var D = this._painters[B];
    if (D) {
        for (var C = 0;
             C < D.length;
    ...
    if (this._active) {
        ...
        var B = function(I) {
            try {
                D.loadText(I.responseText, G, A, C, F);
            } catch(J) {
                SimileAjax.Debug.exception(J);
            } finally {
                H.hideLoadingMessage();
            }
        };
        ...
        window.setTimeout(function() {
            SimileAjax.XmlHttp.get(A, E, B);
        }, 0);
    }
},loadString: function(data, separator, eventSource, format) {
    if (this._active) {
        var tp = this;
        eventSource.loadString(data, separator, format);
    }
},loadXML:function(B, D) {
    if (this._active) {
        var A = this;
...
},putText:function(E, C, A, B) {
    var D = this.putDiv(E, "timeplot-div " + A, B);
    D.innerHTML = C;
    return D;
},putDiv:function(F, B, C) {
    var D = this._id + "-" + F;
    var E = document.getElementById(D);
    if (!E) {
        var A = this._containerDiv.firstChild;
    ...
    this.placeDiv(E, C);
    return E;
},placeDiv:function(B, A) {
    if (A) {
        for (style in A) {
            if (style == "left") {
                A[style] += this._paddingX;
                A[style] += "px";
            } else {
                if (style == "right") {
                    A[style] += this._paddingX;
    ...
    return{x:A.offsetLeft - this._paddingX,y:A.offsetTop - this._paddingY};
},update:function() {
    if (this._active) {
        for (var B = 0;
    ...
    if (this._active) {
        ...
        for (var A = 0;
            ...
            var B = this._plots[A];
            if (B._timeGeometry) {
                B._timeGeometry.reset();
            }
            if (B._valueGeometry) {
                B._valueGeometry.reset();
            }
        }
        this.paint();
    }
    ...
    if (this._active && this._painter == null) {
        ...
        this._painter = window.setTimeout(function() {
            ...
            for (var B = 0;
                 B < C.length;
                 B++) {
                E(C[B].action, C[B].context);
            }
            var D = A._painters.foreground;
            for (var B = 0;
                 B < D.length;
                 B++) {
                E(D[B].action, D[B].context);
            }
            A._painter = null;
        }, 20);
    }
...
},_isBrowserSupported:function(B) {
    var A = SimileAjax.Platform.browser;
    if ((B.getContext && window.getComputedStyle) || (A.isIE && A.majorVersion >= 6)) {
        return true;
    } else {
        return false;
    }
    ...
    if (this._isBrowserSupported(B)) {
        this._clearLabels();
        this._canvas = B;
        B.className = "timeplot-canvas";
        G.appendChild(B);
        if (!B.getContext && G_vmlCanvasManager) {
            B = G_vmlCanvasManager.initElement(this._canvas);
            this._canvas = B;
        }
        this._prepareCanvas();
        ...
        SimileAjax.DOM.registerEvent(C, "click", function() {
            window.location = "http://simile.mit.edu/timeplot/";
        });
        G.appendChild(C);
        var E = this;
        var H = {onAddMany:function() {
            E.update();
        },onClear:function() {
            E.update();
        }};
        ...
        if (this._plotInfos) {
            for (var D = 0;
                 ...
                 D++) {
                var F = new Timeplot.Plot(this, this._plotInfos[D]);
        ...
        this.showLoadingMessage = function() {
            J.containerDiv.style.display = "block";
        };
        this.hideLoadingMessage = function() {
            J.containerDiv.style.display = "none";
        };
        this._active = true;
    } else {
        this._message = SimileAjax.Graphics.createMessageBubble(I);
...
Timeplot.Plot.prototype = {initialize:function() {
    if (this._dataSource && this._dataSource.getValue) {
        ...
        var G = function(I, H, J) {
            if (F._plotInfo.showValues) {
                F._valueFlag.style.display = "block";
                A(I, H, J);
            }
        };
        var B = 24 * 60 * 60 * 1000;
        var E = 30 * B;
        var A = function(J, W, P) {
            if (typeof SimileAjax != "undefined" && F._plotInfo.showValues) {
                var R = F._canvas;
                ...
                if (I < B) {
                    F._timeFlag.innerHTML = Q.toLocaleTimeString();
                } else {
                    if (I > E) {
                        F._timeFlag.innerHTML = Q.toLocaleDateString();
                    } else {
                        F._timeFlag.innerHTML = Q.toLocaleString();
                    }
                }
                ...
                if (F._timeGeometry._timeValuePosition == "top") {
                    F._timeplot.placeDiv(F._valueFlagPole, {left:U,top:H - 5,height:R.height - S - H + 6,display:"block"});
                    F._timeplot.placeDiv(F._timeFlag, {left:M - K,top:-6,display:"block"});
                } else {
                    F._timeplot.placeDiv(F._valueFlagPole, {left:U,bottom:H - 5,height:S - H + 6,display:"block"});
                    F._timeplot.placeDiv(F._timeFlag, {left:M - K,bottom:-6,display:"block"});
                }
                if (U + T + 14 > R.width && S + N + 4 > R.height) {
                    F._valueFlagLineLeft.style.display = "none";
                    ...
                    if (U + T + 14 > R.width && S + N + 4 < R.height) {
                        F._valueFlagLineRight.style.display = "none";
    ...
    if (this._valueFlag) {
        this._valueFlag.style.display = "none";
    }
    if (this._timeFlag) {
        this._timeFlag.style.display = "none";
    }
    if (this._valueFlagLineLeft) {
        this._valueFlagLineLeft.style.display = "none";
    }
    if (this._valueFlagLineRight) {
        this._valueFlagLineRight.style.display = "none";
    }
    if (this._valueFlagPole) {
        this._valueFlagPole.style.display = "none";
    }
    ...
    if (this._dataSource) {
        if (this._plotInfo.fillColor) {
            if (this._plotInfo.fillGradient) {
                ...
                M.fillStyle = this._plotInfo.fillColor.toString();
            }
            M.beginPath();
            M.moveTo(0, 0);
            this._plot(function(T, U) {
                M.lineTo(T, U);
            });
            if (this._plotInfo.fillFrom == Number.NEGATIVE_INFINITY) {
                M.lineTo(this._canvas.width, 0);
            } else {
                if (this._plotInfo.fillFrom == Number.POSITIVE_INFINITY) {
                    M.lineTo(this._canvas.width, this._canvas.height);
                    M.lineTo(0, this._canvas.height);
                } else {
                    M.lineTo(this._canvas.width, this._valueGeometry.toScreen(this._plotInfo.fillFrom));
        ...
        if (this._plotInfo.lineColor) {
            M.strokeStyle = this._plotInfo.lineColor.toString();
            M.beginPath();
            var F = true;
            this._plot(function(T, U) {
                if (F) {
        ...
        if (this._plotInfo.dotColor) {
            M.fillStyle = this._plotInfo.dotColor.toString();
    ...
    if (this._eventSource) {
        var A = M.createLinearGradient(0, 0, 0, this._canvas.height);
        A.addColorStop(1, "rgba(255,255,255,0)");
        M.strokeStyle = A;
        M.fillStyle = A;
        ...
        while (Q.hasNext()) {
            ...
            var O = this;
            var H = function(T) {
                return function(W, U, Z) {
                    var Y = O._timeplot.getDocument();
                    O._closeBubble();
            ...
            var D = function(U, T, V) {
                U.oldClass = U.className;
                U.className = U.className + " timeplot-event-box-highlight";
            };
            var L = function(U, T, V) {
                U.className = U.oldClass;
                U.oldClass = null;
            };
            if (!J.instrumented) {
                SimileAjax.DOM.registerEvent(J, "click", H(P));
    ...
    var E = this._dataSource.getData();
    if (E) {
        var G = E.times;
        var B = E.values;
        var C = G.length;
        for (var D = 0;
             D < C;
             D++) {
            var A = this._timeGeometry.toScreen(G[D]);
...
Timeplot.DefaultEventSource.prototype.loadString = function(text, separator, format) {
...
/* sources.js */
Timeplot.DefaultEventSource = function(A) {
...
Timeplot.DefaultEventSource.prototype.loadText = function(M, I, A, C, K) {
    if (M == null) {
        return;
    }
    this._events.maxValues = new Array();
    ...
    if (G) {
        for (var F = 0;
            ...
            var N = G[F];
            if (N.length > 1) {
                var E = SimileAjax.jQuery.trim(N[0]);
                var D = H(E);
                if (D) {
                    var L = new Timeplot.DefaultEventSource.NumericEvent(D, N.slice(1));
                    this._events.add(L);
                    J = true;
                }
            }
...
Timeplot.DefaultEventSource.prototype._parseText = function(H, C) {
    H = H.replace(/\r\n?/g, "\n");
    var F = 0;
    var E = H.length;
    var I = [];
    while (F < E) {
        ...
        if (H.charAt(F) != "#") {
            while (F < E) {
                ...
                if (G > -1 && G < D) {
                    J[J.length] = H.substr(F, G - F);
                    F = G + 1;
                } else {
                    J[J.length] = H.substr(F, D - F);
                    F = D + 1;
                    break;
                }
            }
...
Timeplot.DefaultEventSource.prototype.getRange = function() {
    var A = this.getEarliestDate();
    var B = this.getLatestDate();
    return{earliestDate:(A) ? A : null,latestDate:(B) ? B : null,min:0,max:0};
};
Timeplot.DefaultEventSource.NumericEvent = function(B, A) {
    this._id = "e" + Math.round(Math.random() * 1000000);
    this._time = B;
    this._values = A;
};
Timeplot.DefaultEventSource.NumericEvent.prototype = {getID:function() {
    return this._id;
},getTime:function() {
    return this._time;
},getValues:function() {
...
Timeplot.DataSource = function(B) {
    this._eventSource = B;
    var A = this;
    this._processingListener = {onAddMany:function() {
        A._process();
    },onClear:function() {
        A._clear();
    }};
...
Timeplot.DataSource.prototype = {_clear:function() {
    this._data = null;
    this._range = null;
},_process:function() {
    ...
    if (this._data) {
        for (var B = 0;
             ...
             B++) {
            var A = this._data.times[B];
            if (A >= C) {
                return this._data.values[B];
            }
        }
    }
    return 0;
},getClosestValidTime:function(B) {
    if (this._data) {
        for (var A = 0;
             A < this._data.times.length;
             A++) {
            var C = this._data.times[A];
            if (C >= B) {
                if (A <= 0) {
                    return C;
                } else {
                    var D = this._data.times[A - 1];
                    if (B - D < C - B) {
                        return D;
                    } else {
                        return C;
    ...
    this._eventSource.addListener(A);
},removeListener:function(A) {
    this._eventSource.removeListener(A);
},replaceListener:function(A, B) {
    this.removeListener(A);
    this.addListener(B);
}};
Timeplot.ColumnSource = function(B, A) {
...
Timeplot.ColumnSource.prototype._getValue = function(A) {
    return parseFloat(A.getValues()[this._column]);
};
Timeplot.ColumnDiffSource = function(C, B, A) {
...
Timeplot.ColumnDiffSource.prototype._getValue = function(C) {
    var B = parseFloat(C.getValues()[this._column]);
...
/* geometry.js */
Timeplot.DefaultValueGeometry = function(A) {
    if (!A) {
        A = {};
    }
    ...
    this._linMap = {direct:function(B) {
        return B;
    },inverse:function(B) {
        return B;
    }};
...
Timeplot.DefaultValueGeometry.prototype = {setTimeplot:function(A) {
    this._timeplot = A;
    this._canvas = A.getCanvas();
    this.reset();
},setRange:function(A) {
    if ((this._minValue == null) || ((this._minValue != null) && (A.min < this._minValue))) {
        this._minValue = A.min;
    }
    if ((this._maxValue == null) || ((this._maxValue != null) && (A.max * 1.05 > this._maxValue))) {
        this._maxValue = A.max * 1.05;
    }
    ...
    if (this._canvas) {
        return this._map.inverse(this._mappedRange * A / this._canvas.height) + this._minValue;
    } else {
        return 0;
    }
},paint:function() {
    if (this._timeplot) {
        ...
        B.lineJoin = "miter";
        if (this._gridColor) {
            var E = B.createLinearGradient(0, 0, 0, this._canvas.height);
            ...
            for (var D = 0;
                ...
                if (typeof C.label != "undefined") {
                    if (this._axisLabelsPlacement == "left") {
                        var F = this._timeplot.putText(this._id + "-" + D, C.label, "timeplot-grid-label", {left:4,bottom:G + 2,color:this._gridColor.toHexString(),visibility:"hidden"});
                        this._labels.push(F);
                    } else {
                        if (this._axisLabelsPlacement == "right") {
                            var F = this._timeplot.putText(this._id + "-" + D, C.label, "timeplot-grid-label", {right:4,bottom:G + 2,color:this._gridColor.toHexString(),visibility:"hidden"});
                            this._labels.push(F);
                        }
                    }
                    if (G + F.clientHeight < this._canvas.height + 10) {
                        F.style.visibility = "visible";
                    }
                }
                B.beginPath();
                if (this._gridType == "long" || C.label == 0) {
                    B.moveTo(0, G);
                    B.lineTo(this._canvas.width, G);
                } else {
                    if (this._gridType == "short") {
                        if (this._axisLabelsPlacement == "left") {
                            B.moveTo(0, G);
                            B.lineTo(this._gridShortSize, G);
                        } else {
                            if (this._axisLabelsPlacement == "right") {
                                B.moveTo(this._canvas.width, G);
    ...
    var C = [];
    if (!this._canvas || this._valueRange == 0) {
        return C;
    }
    var D = 0;
    if (this._valueRange > 1) {
        while (Math.pow(10, D) < this._valueRange) {
            D++;
        }
        D--;
    } else {
    ...
    var F = E;
    while (true) {
        var A = this.toScreen(this._minValue + F);
        while (A < this._gridSpacing) {
            F += E;
            A = this.toScreen(this._minValue + F);
        }
        if (A > 2 * this._gridSpacing) {
            E /= 10;
            F = E;
        } else {
            break;
...
Timeplot.LogarithmicValueGeometry = function(A) {
    Timeplot.DefaultValueGeometry.apply(this, arguments);
    this._logMap = {direct:function(B) {
        return Math.log(B + 1) / Math.log(10);
    },inverse:function(B) {
        return Math.exp(Math.log(10) * B) - 1;
    }};
    this._mode = "log";
...
Timeplot.LogarithmicValueGeometry.prototype._logarithmicCalculateGrid = function() {
    ...
    while (C < this._canvas.height || isNaN(C)) {
        if (C > 0) {
            B.push({y:C,label:A});
        }
        A *= 10;
...
Timeplot.DefaultTimeGeometry = function(B) {
    if (!B) {
        B = {};
    }
    ...
    this._linMap = {direct:function(C) {
        return C;
    },inverse:function(C) {
        return C;
    }};
    ...
    if (this._min && !this._min.getTime) {
        this._min = A(this._min);
    }
    if (this._max && !this._max.getTime) {
        this._max = A(this._max);
    }
    this._labels = [];
    this._grid = [];
};
Timeplot.DefaultTimeGeometry.prototype = {setTimeplot:function(A) {
    this._timeplot = A;
    this._canvas = A.getCanvas();
    this.reset();
},setRange:function(A) {
    if (this._min) {
        ...
        if (A.earliestDate && ((this._earliestDate == null) || ((this._earliestDate != null) && (A.earliestDate.getTime() < this._earliestDate.getTime())))) {
            this._earliestDate = A.earliestDate;
        }
    }
    if (this._max) {
        this._latestDate = this._max;
    } else {
        if (A.latestDate && ((this._latestDate == null) || ((this._latestDate != null) && (A.latestDate.getTime() > this._latestDate.getTime())))) {
            this._latestDate = A.latestDate;
        }
    }
    ...
    if (this._canvas && this._latestDate) {
        var A = C - this._earliestDate.getTime();
    ...
    if (this._canvas) {
        return this._map.inverse(this._mappedPeriod * A / this._canvas.width) + this._earliestDate.getTime();
    } else {
        return 0;
    }
},getPeriod:function() {
    ...
    if (this._canvas) {
        ...
        B.lineJoin = "miter";
        if (this._gridColor) {
            F.addColorStop(0, this._gridColor.toString());
            ...
            for (var D = 0;
                ...
                if (this._axisLabelsPlacement == "top") {
                    var G = this._timeplot.putText(this._id + "-" + D, C.label, "timeplot-grid-label", {left:A + 4,top:2,visibility:"hidden"});
                    this._labels.push(G);
                } else {
                    if (this._axisLabelsPlacement == "bottom") {
                        var G = this._timeplot.putText(this._id + "-" + D, C.label, "timeplot-grid-label", {left:A + 4,bottom:2,visibility:"hidden"});
                        this._labels.push(G);
                    }
                }
                if (A + G.clientWidth < this._canvas.width + 10) {
    ...
    if (B > D.gregorianUnitLengths[D.MILLENNIUM]) {
        G = D.MILLENNIUM;
    } else {
        for (var G = D.MILLENNIUM;
             G > 0;
             G--) {
            if (D.gregorianUnitLengths[G - 1] <= B && B < D.gregorianUnitLengths[G]) {
                G--;
                break;
            }
        }
    ...
    do{
        D.roundDownToInterval(I, G, this._timeZone, 1, 0);
...
Timeplot.MagnifyingTimeGeometry = function(B) {
    Timeplot.DefaultTimeGeometry.apply(this, arguments);
    var A = this;
    this._MagnifyingMap = {direct:function(D) {
        if (D < A._leftTimeMargin) {
            var C = D * A._leftRate;
        } else {
            if (A._leftTimeMargin < D && D < A._rightTimeMargin) {
                var C = D * A._expandedRate + A._expandedTimeTranslation;
            } else {
                var C = D * A._rightRate + A._rightTimeTranslation;
            }
        }
        return C;
    },inverse:function(C) {
        if (C < A._leftScreenMargin) {
            var D = C / A._leftRate;
        } else {
            if (A._leftScreenMargin < C && C < A._rightScreenMargin) {
                var D = C / A._expandedRate + A._expandedScreenTranslation;
            } else {
                var D = C / A._rightRate + A._rightScreenTranslation;
            }
        }
        return D;
    }};
...
Timeplot.MagnifyingTimeGeometry.prototype.initialize = function(F) {
    ...
    var J = function(L, K, M) {
        H._canvas.startCoords = SimileAjax.DOM.getEventRelativeCoordinates(K, L);
        H._canvas.pressed = true;
    };
    var I = function(L, K, N) {
        H._canvas.pressed = false;
        var M = SimileAjax.DOM.getEventRelativeCoordinates(K, L);
        if (Timeplot.Math.isClose(M, H._canvas.startCoords, 5)) {
            H._lens.style.display = "none";
            H.actLinear();
            H._timeplot.paint();
        } else {
            H._lens.style.cursor = "move";
            B(H._lens);
        }
    };
    var D = function(L, K, N) {
        if (H._canvas.pressed) {
            var M = SimileAjax.DOM.getEventRelativeCoordinates(K, L);
            if (M.x < 0) {
                M.x = 0;
            }
            if (M.x > H._canvas.width) {
                M.x = H._canvas.width;
            }
    ...
    var A = function(L, K, M) {
        H._lens.startCoords = SimileAjax.DOM.getEventRelativeCoordinates(K, L);
        H._lens.pressed = true;
    };
    var C = function(L, K, M) {
        H._lens.pressed = false;
    };
    var E = function(M, K, P) {
        if (H._lens.pressed) {
            var N = SimileAjax.DOM.getEventRelativeCoordinates(K, M);
            var L = H._lens;
            var O = L.offsetLeft + N.x - L.startCoords.x;
            if (O < H._timeplot._paddingX) {
                O = H._timeplot._paddingX;
            }
            if (O + L.clientWidth > H._canvas.width - H._timeplot._paddingX) {
                O = H._canvas.width - L.clientWidth + H._timeplot._paddingX;
            }
            L.style.left = O;
            B(L);
        }
    ...
    if (!this._canvas.instrumented) {
        SimileAjax.DOM.registerEvent(this._canvas, "mousedown", J);
    ...
    if (!this._lens.instrumented) {
        SimileAjax.DOM.registerEvent(this._lens, "mousedown", A);
...
Timeplot.MagnifyingTimeGeometry.prototype.setMagnifyingParams = function(F, C, A) {
    C = C / 2;
    A = A / 2;
...
/* color.js */
Timeplot.Color = function(A) {
    this._fromHex(A);
};
Timeplot.Color.prototype = {set:function(D, C, A, B) {
    this.r = D;
    this.g = C;
    this.b = A;
    this.a = (B) ? B : 1;
...
/* math.js */
Timeplot.Math = {range:function(G) {
    var D = G.length;
...
},isClose:function(B, A, C) {
    return(B && A && Math.abs(B.x - A.x) < C && Math.abs(B.y - A.y) < C);
}};


/* processor.js */
Timeplot.Operator = {sum:function(A, B) {
    return Timeplot.Math.integral(A.values);
},average:function(C, D) {
    var B = ("size" in D) ? D.size : 30;
    var A = Timeplot.Math.movingAverage(C.values, B);
    return A;
}};
Timeplot.Processor = function(D, A, C) {
    this._dataSource = D;
    this._operator = A;
    this._params = C;
    this._data = {times:new Array(),values:new Array()};
...
Timeplot.Processor.prototype = {_clear:function() {
    this.removeListener(this._processingListener);
    this._dataSource._clear();
},_process:function() {
    var D = this._dataSource.getData();
    ...
    return this._data;
},getValue:Timeplot.DataSource.prototype.getValue,getClosestValidTime:Timeplot.DataSource.prototype.getClosestValidTime,addListener:function(A) {
    this._dataSource.addListener(A);
},removeListener:function(A) {
    this._dataSource.removeListener(A);
}};
